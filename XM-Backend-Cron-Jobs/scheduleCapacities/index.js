...
module.exports = async function (context, myTimer) {
    ...
    try{
        ...
        const USERISSUEREQUEST = await axios.post(`${server}/api/orchestrators?type=CAPACITIES`, {}).then(response => {
        ...
        const ONBOARDING_USERSUPDATE = await myDB.collection(DB_ONBOARDING).updateOne({tag: DB_ONBOARDING_DOC_USERS }, {
            $set: {"hour.bulkCapacity":timeStamp}
        });
        ...
        // console.log('err', err)
        const ONBOARDING_USERSUPDATE = await myDB.collection(DB_ONBOARDING).updateOne({tag: DB_ONBOARDING_DOC_USERS }, {
            $push: {"hour.errorCapacity":"err: " + JSON.stringify(err)}
        });
