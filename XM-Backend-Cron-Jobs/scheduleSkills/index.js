...
module.exports = async function (context, myTimer) {
    ...
    try{
        ...
        const USERISSUEREQUEST = await axios.post(`${server}/api/orchestrators?type=SKILLS`, {}).then(response => {
        ...
        // console.log('err', err)
        const ONBOARDING_USERSUPDATE = await myDB.collection(DB_ONBOARDING).updateOne({tag: DB_ONBOARDING_DOC_USERS }, {
            $push: {"hour.errorSkill":"err: " + JSON.stringify(err)}
        });
