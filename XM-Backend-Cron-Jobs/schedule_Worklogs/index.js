...
module.exports = async function (context, myTimer) {
    ...
    try{
        ...
        const USERISSUEREQUEST = await axios.post(`${server}/api/orchestrators?type=WORKLOGS`, {}).then(response => {
        ...
        const ONBOARDING_USERSUPDATE = await myDB.collection(DB_ONBOARDING).updateOne({tag: DB_ONBOARDING_DOC_USERS }, {
            $set: {
                // [`records.${timeStamp}`]:{
                //     start:timeStamp,
                //     status: USERISSUEREQUEST.status.body.statusQueryGetUri || ''
                // },
                "hour.bulkWorklog":timeStamp
            }
        });
        ...
        // console.log('err', err)
        const ONBOARDING_USERSUPDATE = await myDB.collection(DB_ONBOARDING).updateOne({tag: DB_ONBOARDING_DOC_USERS }, {
            $push: {"hour.errorWorklog":"err: " + JSON.stringify(err)}
        });
