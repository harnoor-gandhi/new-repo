...
module.exports = async function (context, myTimer) {
    ...
    console.log("Issue Piranhas start")


        try{
            let server = (isPROD == "PROD" ? isProdBackeURL: (isPROD == "DEV" ? isDevBackeURL:isLocalBackeURL) );

            const USERISSUEREQUEST = await axios.post(`${server}/api/orchestrators?type=ISSUES50`, {}).then(response => {
            ...
            const ONBOARDING_USERSUPDATE = await myDB.collection(DB_ONBOARDING).updateOne({tag: DB_ONBOARDING_DOC_USERS }, {
                $set: {"hour.bulkPiranha":timeStamp}
            });
            ...
            // console.log('err', err)
            const ONBOARDING_USERSUPDATE = await myDB.collection(DB_ONBOARDING).updateOne({tag: DB_ONBOARDING_DOC_USERS }, {
                $push: {"hour.errorPiranha":"err: " + JSON.stringify(err)}
            });
