...
module.exports = async function (context, myTimer) {
    ...
    console.log("Issue Turtles start")


        try{
            let server = (isPROD == "PROD" ? isProdBackeURL: (isPROD == "DEV" ? isDevBackeURL:isLocalBackeURL) );

            const USERISSUEREQUEST = await axios.post(`${server}/api/orchestrators?type=ISSUES5000`, {}).then(response => {
            ...
            const ONBOARDING_USERSUPDATE = await myDB.collection(DB_ONBOARDING).updateOne({tag: DB_ONBOARDING_DOC_USERS }, {
                $set: {"hour.bulkTurtle":timeStamp}
            });

            // console.log('JavaScript 24h timer trigger function ran!', timeStamp);
            return {success:true, done:USERISSUEREQUEST} 
        }catch(err){

            // console.log('err', err)
            const ONBOARDING_USERSUPDATE = await myDB.collection(DB_ONBOARDING).updateOne({tag: DB_ONBOARDING_DOC_USERS }, {
                $push: {"hour.errorTurtle":"err: " + JSON.stringify(err)}
            });
